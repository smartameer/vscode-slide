...
export class ScoreProvider implements vscode.TreeDataProvider<Dependency> {
  public static readonly viewType = 'slide.scoreboard'
  ...
  parseTreeData(items: any[]): Dependency[] {
    ...
    for (var i in items) {
      const d = items[i]
      const date = (new Date(d.winningDate)).toDateString()
      list[date] = list[date] ?? []
      list[date].push(d)
    }
  ...
  getChildren(element?: Dependency): Thenable<Dependency[]> {
    ...
    if (typeof element.scores !== "undefined") {
        ...
        const timeObj = new Date(item.winningDate)
				return new Dependency({
					title: timeObj.getHours().toString().padStart(2, '0') + ':' + timeObj.getMinutes().toString().padStart(2, '0'),
					description: `\tLevel: ${item.gameLevel} x ${item.gameLevel}\t Moves: ${item.moves}`,
          tooltip: `Date: ${timeObj.toLocaleString()}\nLevel: ${item.gameLevel} x ${item.gameLevel}\nMoves: ${item.moves}`
